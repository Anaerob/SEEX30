...
def main():
    ...
    # Initialize
    progressExist = os.path.isfile('MCTS_sweepD_progress.txt')
    if progressExist:
        array = Load.loadFloatArray('MCTS_sweepD_progress', 7)
    ...
    for iDepth in range(startDepth, len(depth)):
        ...
        if not progressExist:
            ...
            tie = 0
            win = 0
        abortFile = open('MCTS_sweepD_abort.txt', 'a')
        lossFile = open('MCTS_sweepD_loss.txt', 'a')
        tieFile = open('MCTS_sweepD_tie.txt', 'a')
        timeFile = open('MCTS_sweepD_time.txt', 'a')
        winFile = open('MCTS_sweepD_win.txt', 'a')
        ...
        for iGame in range(startGame, nGames):
            ...
            progressFile = open('MCTS_sweepD_progress.txt', 'w')
            progressFile.write(
                str(iDepth) + '\n'
                + str(iGame + 1) + '\n'
                + str(abort) + '\n'
        ...
        progressExist = False
    print('Completed 100%')
    os.remove('MCTS_sweepD_progress.txt')

if __name__ == '__main__':
    mainTime = time.time()
    main()
    PrintTime.printTime(time.time() - mainTime)
