...
import Game
import Load
import MonteCarloLinearNeuralSearchAI
import RandomAI
...
def main():
    ...
    # Initialize
    AI = [MonteCarloLinearNeuralSearchAI.AI(limit, minimum, search), RandomAI.AI()]
    if not os.path.isdir('MCLNS_benchmark'):
        os.makedirs('MCLNS_benchmark')
    progressExist = os.path.isfile('MCLNS_benchmark/progress.txt')
    if progressExist:
        array = Load.loadFloatArray('MCLNS_benchmark/progress', 5)
        ...
        if os.path.isfile('MCLNS_benchmark/results.txt'):
        ...
        tie = 0
        win = 0
    infoFile = open('MCLNS_benchmark/info.txt', 'w')
    infoFile.write(
        'Games = ' + str(nGames) + '\n'
        + 'Limit = ' + str(limit) + '\n'
        + 'Minimum = ' + str(minimum) + '\n'
    ...
    for iGame in range(startGame, nGames):
        ...
        if game.win[0] and game.win[1]:
            ...
            abort += 1
        progressFile = open('MCLNS_benchmark/progress.txt', 'w')
        progressFile.write(
            str(iGame + 1) + '\n'
            + str(abort) + '\n'
            + str(loss) + '\n'
            ...
            + str(win))
        progressFile.close()
    resultsFile = open('MCLNS_benchmark/results.txt', 'w')
    resultsFile.write(
    ...
    resultsFile.close()
    print('Completed 100%')
    os.remove('MCLNS_benchmark/progress.txt')
